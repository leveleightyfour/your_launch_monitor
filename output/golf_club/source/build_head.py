...
neck=hosel_tube('Hosel • integrated tapered neck',base,axis,[(-.009,0),(-.005,.003),(0,.005),(.007,.0080),(.014,.0095),(.021,.0080),(.024,.0075)],lacquer)
# Fuse the neck into the shell and sculpt the actual intersection region.
# The head and neck share topology throughout the transition.
from mathutils.kdtree import KDTree
bm=bmesh.new();bm.from_mesh(neck.data)
bmesh.ops.holes_fill(bm,edges=[e for e in bm.edges if e.is_boundary],sides=0)
bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces))
bm.to_mesh(neck.data);bm.free();neck.data.update()
old_positions=[v.co.copy() for ob in [body,neck] for v in ob.data.vertices]
old_tree=KDTree(len(old_positions))
for i,p in enumerate(old_positions):old_tree.insert(p,i)
old_tree.balance()
bpy.ops.object.select_all(action='DESELECT');body.select_set(True)
bpy.context.view_layer.objects.active=body
union=body.modifiers.new('Fuse hosel into head','BOOLEAN')
union.operation='UNION';union.solver='EXACT';union.object=neck
bpy.ops.object.modifier_apply(modifier=union.name)
bpy.data.objects.remove(neck,do_unlink=True)
bm=bmesh.new();bm.from_mesh(body.data)
new_verts={v for v in bm.verts if old_tree.find(v.co)[2]>1e-7}
seam=[e for e in bm.edges if all(v in new_verts for v in e.verts) and len(e.link_faces)==2 and e.calc_face_angle(0)>.08]
print('Hosel fillet intersection edges:',len(seam))
assert len(seam)>12,'No usable head/hosel intersection loop was found'
# Uniform local topology lets the surrounding crown participate in the blend.
# The UV-bearing source is retained for material classification after remeshing.
seam_points=[v.co.copy() for e in seam for v in e.verts]
seam_tree=KDTree(len(seam_points))
for i,p in enumerate(seam_points):seam_tree.insert(p,i)
seam_tree.balance()
bm.free()
from mathutils.bvhtree import BVHTree
source_mesh=body.data.copy()
source_materials=list(body.data.materials)
source_tree=BVHTree.FromPolygons([v.co.copy() for v in source_mesh.vertices],[list(p.vertices) for p in source_mesh.polygons],all_triangles=False)
body.data.remesh_voxel_size=.00035
body.data.use_remesh_preserve_volume=True
bpy.ops.object.voxel_remesh()
blend_group=body.vertex_groups.new(name='Hosel transition • local sculpt')
for v in body.data.vertices:
    d=seam_tree.find(v.co)[2]
    height=(v.co-base).dot(axis)
    top_fade=max(0.0,min(1.0,(.0160-height)/.0040))
    weight=math.exp(-(d/.0085)**4)*top_fade
    if weight>.001:blend_group.add([v.index],weight,'REPLACE')
smooth=body.modifiers.new('Sculpt the neck into the crown','SMOOTH')
smooth.factor=.82;smooth.iterations=85;smooth.vertex_group=blend_group.name
bpy.ops.object.modifier_apply(modifier=smooth.name)
reduce=body.modifiers.new('Mobile mesh reduction','DECIMATE')
reduce.ratio=.065
bpy.ops.object.modifier_apply(modifier=reduce.name)
# Project the untouched head area back onto its original surface.
for vertex in body.data.vertices:
    d=seam_tree.find(vertex.co)[2]
    height=(vertex.co-base).dot(axis)
    blend=(1.0 if d<.012 else max(0,1-(d-.012)/.006))*max(0,min(1,(.0160-height)/.0040))
    nearest,_,_,_=source_tree.find_nearest(vertex.co)
    vertex.co=vertex.co*blend+nearest*(1-blend)
body.data.materials.clear()
for m in source_materials:body.data.materials.append(m)
uv_layer=body.data.uv_layers.get('UVMap') or body.data.uv_layers.new(name='UVMap')
for poly in body.data.polygons:
    centre=sum((body.data.vertices[i].co for i in poly.vertices),Vector())/len(poly.vertices)
    _,_,source_index,_=source_tree.find_nearest(centre)
    mat_index=source_mesh.polygons[source_index].material_index
    poly.material_index=mat_index;poly.use_smooth=True
    mat=source_materials[mat_index]
    for li in poly.loop_indices:
        p=body.data.vertices[body.data.loops[li].vertex_index].co
        if mat==sole_mat:
            px=650-(p.y+.041)/.109*554;py=321-(p.x-.002)/.063*308
            uv_layer.data[li].uv=(px/755,1-py/1017)
        elif mat==face_mat:
            px=260-(p.x-.002)/PIXEL_SCALE;py=724-(p.z-.002)/PIXEL_SCALE
            uv_layer.data[li].uv=((px-60)/418,1-(py-615)/219)
        else:uv_layer.data[li].uv=(0,0)
# Keep the exact face-insert boundary instead of a voxel stair-step edge.
src_bm=bmesh.new();src_bm.from_mesh(source_mesh)
insert_index=source_materials.index(face_mat)
insert_edges=[(e.verts[0].co.copy(),e.verts[1].co.copy()) for e in src_bm.edges if len(e.link_faces)==2 and (e.link_faces[0].material_index==insert_index)!=(e.link_faces[1].material_index==insert_index)]
src_bm.free()
bm=bmesh.new();bm.from_mesh(body.data)
border_verts={v for e in bm.edges if len(e.link_faces)==2 and (e.link_faces[0].material_index==insert_index)!=(e.link_faces[1].material_index==insert_index) for v in e.verts}
for vertex in border_verts:
    nearest=None;distance=1e9
    for a,b in insert_edges:
        edge=b-a;t=max(0,min(1,(vertex.co-a).dot(edge)/edge.length_squared));q=a+edge*t
        d=(vertex.co-q).length_squared
        if d<distance:distance=d;nearest=q
    if nearest is not None and distance<.0015**2:vertex.co=nearest
bmesh.ops.remove_doubles(bm,verts=list(bm.verts),dist=1e-7)
bmesh.ops.dissolve_degenerate(bm,dist=1e-8,edges=list(bm.edges))
# Keep enough curvature samples around the thin polished face rim for clean highlights.
rim_edges=[e for e in bm.edges if all(v.co.y<-.033 and v.co.z>.009 and (v.co-base).length>.025 for v in e.verts)]
bmesh.ops.subdivide_edges(bm,edges=rim_edges,cuts=2,use_grid_fill=True)
for vertex in bm.verts:
    if vertex.co.y<-.032 and vertex.co.z>.008:
        d=seam_tree.find(vertex.co)[2];height=(vertex.co-base).dot(axis)
        blend=(1.0 if d<.012 else max(0,1-(d-.012)/.006))*max(0,min(1,(.0160-height)/.0040))
        q,_,_,_=source_tree.find_nearest(vertex.co)
        vertex.co=vertex.co*blend+q*(1-blend)
bmesh.ops.remove_doubles(bm,verts=list(bm.verts),dist=1e-7)
bmesh.ops.dissolve_degenerate(bm,dist=1e-7,edges=list(bm.edges))
# Resolve sub-micron slivers explicitly before glTF's triangulation.
for _ in range(4):
    bmesh.ops.triangulate(bm,faces=list(bm.faces))
    slivers=[f for f in bm.faces if f.calc_area()<5e-14]
...
bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces))
assert all(len(e.link_faces)==2 for e in bm.edges),'Fused head must be closed and manifold'
bm.to_mesh(body.data);bm.free();body.data.update()
# Refresh sculpt weights after topology changes so new rim vertices retain
# the smooth junction normals instead of sampling the old cylinder seam.
for vertex in body.data.vertices:
    d=seam_tree.find(vertex.co)[2];height=(vertex.co-base).dot(axis)
    blend=(1.0 if d<.012 else max(0,1-(d-.012)/.006))*max(0,min(1,(.0160-height)/.0040))
    blend_group.add([vertex.index],blend,'REPLACE')
# UVs follow the restored boundary exactly.
uv_layer=body.data.uv_layers['UVMap']
for poly in body.data.polygons:
    mat=source_materials[poly.material_index]
    for li in poly.loop_indices:
        p=body.data.vertices[body.data.loops[li].vertex_index].co
        if mat==sole_mat:
            uv_layer.data[li].uv=((650-(p.y+.041)/.109*554)/755,1-(321-(p.x-.002)/.063*308)/1017)
        elif mat==face_mat:
            uv_layer.data[li].uv=((260-(p.x-.002)/PIXEL_SCALE-60)/418,1-(724-(p.z-.002)/PIXEL_SCALE-615)/219)
normal_source=bpy.data.objects.new('Temporary original surface for normals',source_mesh)
studio_col.objects.link(normal_source)
body.name='Head and hosel • continuous sculpted shell'
body['hosel_transition']='One closed surface; local sculpted blend from crown into neck'
body.select_set(False)
...
# Preserve the original smooth crown/face normals outside the sculpted junction.
for vertex in source_mesh.vertices:vertex.co.x*=-1
bm=bmesh.new();bm.from_mesh(source_mesh)
bmesh.ops.reverse_faces(bm,faces=list(bm.faces));bm.to_mesh(source_mesh);bm.free();source_mesh.update()
bpy.ops.object.select_all(action='DESELECT');body.select_set(True);bpy.context.view_layer.objects.active=body
print('Smoothing final surface faces:',sum(not p.use_smooth for p in body.data.polygons))
for poly in body.data.polygons:poly.use_smooth=True
body.data.update()
normals=body.modifiers.new('Preserve original head shading','DATA_TRANSFER')
normals.object=normal_source;normals.use_loop_data=True;normals.data_types_loops={'CUSTOM_NORMAL'}
normals.loop_mapping='POLYINTERP_NEAREST';normals.vertex_group=blend_group.name;normals.invert_vertex_group=True
bpy.ops.object.modifier_apply(modifier=normals.name)
# A single normal per welded vertex keeps the transferred shading continuous
# across the new triangles and across UV/material boundaries.
vertex_normals=[Vector((0,0,0)) for _ in body.data.vertices]
for loop,normal in zip(body.data.loops,body.data.corner_normals):
    vertex_normals[loop.vertex_index]+=normal.vector
for n in vertex_normals:n.normalize()
body.data.normals_split_custom_set_from_vertices(vertex_normals)
body.data.update()
...
'hosel':camera('06 • Hosel blend detail',(-.035,-.15,.080),(.043,-.026,.026),.072),
}
