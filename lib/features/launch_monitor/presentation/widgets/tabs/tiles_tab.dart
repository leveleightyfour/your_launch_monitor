...
/// The metric formatters used to live in this file. They now sit in the
/// application layer so the Apple Watch bridge can ship exactly the strings
/// the tiles draw; re-exported here so existing imports keep working.
export 'package:omni_sniffer/features/launch_monitor/application/tile_formatting.dart';
...
class _MetricTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String footer = '';
    if (isImpact) {
      final hAvg = fmtImpactH(avgShot?.horizontalImpact);
      final vAvg = fmtImpactV(avgShot?.verticalImpact);
...
class _ImpactDisplay extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final hStr = fmtImpactH(shot?.horizontalImpact);
    final vStr = fmtImpactV(shot?.verticalImpact);
