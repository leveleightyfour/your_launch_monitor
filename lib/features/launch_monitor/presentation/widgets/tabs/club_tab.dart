...
class _TrajPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final offlineStr =
        '${offline >= 0 ? 'R' : 'L'}${offline.abs().toStringAsFixed(1)}';
...
class _TrajPainter extends CustomPainter {
  ...
  bool shouldRepaint(_TrajPainter old) =>
      old.shot != shot || old.accent != accent;
}
        ...
        Positioned.fill(
          child: Align(
            ...
            child: Column(
              ...
              children: [
                Text(
                  '${shot.launchAngle.toStringAsFixed(1)}°',
...
class _LoftPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final aoaEnd = Offset(
      ...
      oy +
          (aoaDeg < 0 ? aoaLen * math.sin(aoaRad) : -aoaLen * math.sin(aoaRad)),
    );
    canvas.drawLine(
      Offset(ox, oy),
      aoaEnd,
...
class _ImpactSection extends StatelessWidget {
  ...
  static List<double> _faceBounds(ClubType type, String? id) {
    ...
    return switch (type) {
      ClubType.wood || ClubType.miniDriver => [0.064, 0.723, 0.664, 0.903],
      ClubType.hybrid => [0.087, 0.732, 0.627, 0.912],
      ClubType.iron => [0.092, 0.717, 0.632, 0.917],
      ClubType.wedge || ClubType.putter => [0.058, 0.725, 0.558, 0.925],
    };
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: active ? context.accent : AppColors.border2),
      ),
      child: Stack(
        children: [
          AnimatedPositioned(
